-- ⚠️ RUN THIS IN SUPABASE SQL EDITOR TO FIX PER-USER LIMIT VALIDATION ⚠️
-- This fixes the issue where users can apply the same discount multiple times
...
-- Show current usage records
SELECT '---' as "=====";
SELECT 'Recent usage records:' as info;
SELECT
    dc.code as discount_code,
    du.user_id,
    COUNT(*) as times_used,
    dc.user_usage_limit as limit,
    CASE
        WHEN COUNT(*) >= dc.user_usage_limit THEN '❌ LIMIT REACHED'
        ELSE '✅ CAN STILL USE'
    END as status
FROM discount_usage du
JOIN discount_codes dc ON dc.id = du.discount_id
WHERE dc.user_usage_limit IS NOT NULL
GROUP BY dc.code, du.user_id, dc.user_usage_limit
ORDER BY times_used DESC;

SELECT '---' as "=====";
SELECT '✅ Migration complete! Per-user limits should now be enforced correctly.' as status;