-- =============================================
-- UPDATED get_products_enhanced with Variant Discount Support
-- Each variant now includes its own discount information
-- =============================================

-- This DROP is now in a separate file (drop_all_get_products_enhanced.sql)
-- Run that file first, then run this file

-- Helper function to get product-level best discount
...
BEGIN
  ...
  -- First, get variant discounts in a separate CTE for better performance
    ...
    SELECT
      ...
      JSONB_AGG(
        JSONB_BUILD_OBJECT(
          ...
          -- Add variant-specific discount info from the CTE
  ...
  -- Get product-level discount (best discount across all variants or product-level discount)
  product_discount AS (
    SELECT
      p.id,
  ...
  SELECT
    ...
    -- Use product-level discount OR if any variant has discount
    ...
    -- Additional fields including translations
    p.is_featured,
    p.category_id,
    p.description,
    p.meta_title,
...
COMMENT ON FUNCTION get_products_enhanced IS 'Enhanced product listing with variant-specific discount support. Each variant includes has_discount, discount_percentage, and discounted_price fields.';
