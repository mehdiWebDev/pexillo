...
-- Now let's fix the get_products_enhanced function
...
BEGIN
  ...
  filtered_products AS (
    ...
    WHERE p.is_active = true
      ...
      AND (NOT on_sale_only OR p.id IN (SELECT id FROM products_with_discounts))
      AND (
        size_filter IS NULL
        OR EXISTS (
  ...
  product_variants_agg AS (
    SELECT
      ...
      JSONB_AGG(
        ...
        ORDER BY pv.display_order, pv.size, pv.color
      ) AS variants,
  ...
  ORDER BY
    CASE
      WHEN sort_by = 'price' AND sort_order = 'ASC' THEN p.base_price
      WHEN sort_by = 'price' AND sort_order = 'DESC' THEN -p.base_price
      WHEN sort_by = 'name' AND sort_order = 'ASC' THEN p.name::TEXT
      WHEN sort_by = 'name' AND sort_order = 'DESC' THEN p.name::TEXT
      WHEN sort_by = 'rating' THEN -p.average_rating
      WHEN sort_by = 'popular' THEN -p.review_count
      ELSE NULL
    END NULLS LAST,
    CASE
